...
function search() {
    ...
    for (x = 0; x < cityHistory.length; x++) {
        ...
        historyBtn.addEventListener("click", function(event){buttonSearch(event.target.innerHTML)});
    }
...
function buttonSearch(cityinput) {
    ...
    for (x = 0; x < cityHistory.length; x++) {
        ...
        historyBtn.textContent = cityHistory[x];
        document.body.querySelector("ul").appendChild(historyBtn)
        historyBtn.addEventListener("click", secsearch());
    }
